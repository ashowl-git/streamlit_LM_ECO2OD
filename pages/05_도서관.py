...
df_raw = pd.read_excel('data/OD_data.xlsx', sheet_name='05_Library')
...
def user_input_features():
    ...
    외벽= st.sidebar.slider('외벽', 0.0, 6.0, 0.5 )
    지붕 = st.sidebar.slider('지붕', 0.0, 6.0, 0.35)
    바닥 = st.sidebar.slider('바닥', 0.0, 6.0, 0.5)
    창호열관류율 = st.sidebar.slider('창호열관류율', 0.0, 6.0, 3.8 )
    SHGC = st.sidebar.slider('SHGC', 0.0, 2.0, 0.717)
    문열관류율 = st.sidebar.slider('문열관류율', 0.0, 6.0, 3.5 )
    난방효율 = st.sidebar.slider('난방효율', 0.0, 7.0, 3.0)
    냉방효율 = st.sidebar.slider('냉방효율', 0.0, 7.0, 3.0 )
    급탕효율 = st.sidebar.slider('급탕효율', 0.0, 100.0, 80.0 )
    조명밀도 = st.sidebar.slider('조명밀도',  0.0, 20.0, 7.1, )
